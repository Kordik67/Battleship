...
class ShotGrid(Entity):
    def __init__(self, cli, **kwargs):
        ...
        self.cells = \
            [
                [
                    Entity(parent=self, model="quad", scale=(1, 1),
                           color=color.white33, collider="box")
                    for x in range(10)
                ]
                for y in range(10)
            ]
        ...
        # print(self.cells)
        self.fixed = False
        self.shot_boutton = Button(
            text='Tirer', scale=(.1, .1), position=(-.5, 0))
    ...
    def shot_responce(self, hit):
        if hit and self.target is not None:
            self.target.color = color.red
            Animation('assets/explosion.gif', loop=False,
                      parent=self.target, z=-0.1)
