                ...
                if message == b'Connected':
                    ...
                    match message[0]:
                        ...
                        case 2:  # SET PLAYER NAME
                            self.players[address[0]
                                         ].name = message[1:].decode()
                        case 3:  # START GAME
                            start_game = b"\x01"
                            ...
                            if message[1]:  # PLAY VS AI
                                ...
                                pass
                            self.send(
                                start_game, self.games_waiting[player.name].players)
                        ...
                        case 4:  # PLACE BOAT
                            ...
                            for i in range(len(msg)//4):
                                ...
                                g.place_boat(player, (x, y),
                                             VECTOR[dir_vec], lenght)
                        case 5:  # REQUEST GAMES
                            pack = b"\x03"
                            pack += b"\x00".join([name.encode()
                                                 for name in self.games_waiting.keys()])
                            pack += b"\x01"
                            pack += b"\x00".join([name.encode()
                                                 for name in self.games_runing.keys()])
